...
if (typeof(Components) != "undefined" && typeof(JSIO) != 'boolean') {
	...
	var FileIO = {
		...
		write  : function(file, data, mode, charset) {
			try {
				...
				foStream.init(file, flags, 664, 0);
				if (charset) {
					data = this.fromUnicode(charset, data, foStream);
				} else {
		...
		/* Code for version before Gecko 1.8
        ...
        fromUnicode : function(charset, data, foStream) {
            try {
                var os = Components.classes[this.coutstreamCID]
